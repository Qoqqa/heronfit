...
class CollaborativeRecommendationService extends BaseRecommendationService {
  ...
  final ContentBasedRecommendationService _fallbackService = ContentBasedRecommendationService();
  ...
  Future<List<Workout>> getRecommendations(String? userId, {int count = 4}) async {
    ...
    try {
      ...
      final similarUserIds = userSimilarity.entries
          .toList()
          ..sort((a, b) => b.value.compareTo(a.value));
          
      final topSimilarUserIds = similarUserIds
      ...
      final similarUsersWorkouts = 
          await _exerciseService.getSimilarUsersWorkouts(topSimilarUserIds);
      
      // 7. Recommend workouts user hasn't done yet
      final recommendations = <Workout>[];
      final userWorkoutExerciseSets = userWorkouts.map(
        (w) => Set<String>.from((w['exercises'] as List).cast<String>())
      ).toList();
