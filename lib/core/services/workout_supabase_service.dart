...
class WorkoutSupabaseService {
  ...
  Future<void> deleteWorkoutTemplate(String templateId) async {
    try {
      // Attempt to delete the template and use .single() to ensure one row is affected.
      // This will throw a PostgrestException if zero or multiple rows match,
      // which is the intended behavior for ensuring a single delete.
      final response =
          await _supabase
              .from('workout_templates')
              .delete()
              .eq('id', templateId)
              .select(); // Use select() to get the affected rows

      if (response == null || response.isEmpty) {
        // If response is null or empty, it means no row was found matching the criteria.
        // We can throw a specific error or handle it gracefully.
        debugPrint('No workout template found with ID: $templateId to delete.');
        throw PostgrestException(
          message: 'Workout template not found',
          code: '404',
          details: 'No template matched the provided ID and user.',
          hint: null,
        ); // Throw a more specific error
      } else if (response.length > 1) {
        // This case should ideally not happen with RLS and unique IDs, but good to check.
        debugPrint('Warning: Multiple templates found with ID: $templateId.');
        throw PostgrestException(
          message: 'Multiple templates found',
          code: '500',
          details: 'Multiple templates matched the provided ID.',
          hint: null,
        ); // Indicate unexpected state
      } else {
        // Exactly one row was deleted, which is the expected successful case.
        debugPrint('Workout template deleted from Supabase: $templateId');
      }
      // Note: .single() is removed here because we are manually checking the response length.
      // Using .select() then checking the list is a more explicit way to handle 0 or >1 results
      // compared to relying solely on .single()'s exception handling.
    } catch (e) {
