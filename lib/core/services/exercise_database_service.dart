...
class ExerciseDatabaseService {
  ...
  Future<List<String>> getRandomExercisesByCategory(
    String category,
    int count,
  ) async {
    try {
      ...
      // Otherwise, get random exercises regardless of category
        ...
        allData.shuffle();
        return allData
            .sublist(0, min(count, allData.length))
  ...
  Future<List<Map<String, dynamic>>> getUserWorkoutHistory(
    String userId, {
    int limit = 10,
  }) async {
    try {
  ...
  Future<List<Map<String, dynamic>>> getSimilarUsersWorkouts(
    List<String> userIds, {
    int limit = 20,
  }) async {
    try {
  ...
  /// Get Exercise objects by their names
  Future<List<Exercise>> getExercisesByNames(List<String> names) async {
    ...
    try {
      final response = await Supabase.instance.client
          .from('exercises')
          .select() // Select all columns to build Exercise objects
          .inFilter('name', names);

      return response.map((json) => Exercise.fromJson(json)).toList();
    } catch (e) {
      print('Error getting exercises by names: $e');
  ...
  String _mapCategoryToExerciseCategory(String workoutCategory) {
    switch (workoutCategory) {
      ...
      case 'Cardio':
        return 'cardio';
      default:
        return ''; // Will trigger fallback to random exercises
    }
  }
