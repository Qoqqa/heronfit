...
class ContentBasedRecommendationService extends BaseRecommendationService {
  ...
  final RandomRecommendationService _fallbackService =
      RandomRecommendationService();
    ...
    try {
      ...
      for (var workout in userWorkouts) {
        ...
        final category =
            ...
            _guessCategoryFromExercises(exercises);
  ...
  Future<List<String>> _getExercisesBasedOnPreferences(
    ...
    if (result.length < count) {
      final categoryExercises = await _exerciseService
          .getRandomExercisesByCategory(category, count - result.length);
  ...
  String _guessCategoryFromExercises(List<String> exercises) {
    ...
    if (lowerCaseExercises.any(
      ...
      return 'Upper Body';
    } else if (lowerCaseExercises.any(
      (e) =>
          e.contains('squat') ||
          e.contains('lunge') ||
          e.contains('leg') ||
      ...
      (e) => e.contains('crunch') || e.contains('plank') || e.contains('ab'),
    )) {
      return 'Core';
    } else if (lowerCaseExercises.any(
      (e) => e.contains('run') || e.contains('sprint') || e.contains('jog'),
    )) {
      return 'Cardio';
    }
